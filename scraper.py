        ...
        try:
            ...
            for elem in elements[:2]:
                ...
                if attr and 'http' in attr and ('m3u8' in attr or 'hls' in attr or 'stream' in attr):
                    # Agrega token mock si no hay (para test refresh)
                    if '?' not in attr:
            ...
            continue
    # Fallback mock si 0 streams (para player test)
    if not streams:
        streams = [
            {
...
def run_scraper():
    # Fecha pasada para test (2024-11-03: Tottenham vs Aston Villa, etc.)
    today = '2024-11-03'
